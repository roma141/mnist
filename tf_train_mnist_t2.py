...
categories = 10
stamp = "mnist_t2_m2_hid-"+str(hidden)+"_cs-"+str(cv_size)+"_chan-"+str(cv_channels)
...
h_aver_pool = aver_pool(h_pool2, 7)

h_pool_flat = tf.reshape(h_aver_pool, [-1, img_width/4 * img_height/4  * cv_channels * 2])
...
h_aver_pool_cv = aver_pool(h_pool2_cv, 7)

h_pool_flat_cv = tf.reshape(h_aver_pool_cv, [-1, img_width/4 * img_height/4  * cv_channels * 2])
