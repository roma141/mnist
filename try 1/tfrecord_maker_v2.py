...
directory_to_save = "/media/roma141/Alma 2TB/torrents/mnist/normal-x8"
#group = "test"
group = "train"
test_split = 0.1
test_list = {}
...
for key in count_by_label:
	...
	for temp in all_train[key]:
		...
		for t in xrange(8):
			...
			if t == 2:
				img = img.rotate(20, Image.BICUBIC)
			if t == 3:
				img = img.rotate(20, Image.BICUBIC)
				img = ImageOps.mirror(img)
			if t == 4:
				img = img.rotate(40, Image.BICUBIC)
			if t == 5:
				img = img.rotate(40, Image.BICUBIC)
				img = ImageOps.mirror(img)
			if t == 6:
				img = img.rotate(60, Image.BICUBIC)
			if t == 7:
				img = img.rotate(60, Image.BICUBIC)
				img = ImageOps.mirror(img)
		# 	ax = plt.subplot(a, b, i, frame_on=False)
		# 	if i >= a*b:
		# 		pass
		# 	else:
		# 		i += 1
		...
		# plt.show()
			img = np.array(img)
			file_name = key + "_" + str(count) + "_" + str(t)
