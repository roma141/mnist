...
f_loss, f_acc = test(model, test_loader)
name_file = "pytorch_typ2_loss-" + str(round(f_loss,4))+ "_acc-" + str(round(f_acc,2))
torch.save(model.state_dict(), name_file +".pt")

file_all = "../data/"
file_name = file_all + 'train-tr.csv'
test_loader = torch.utils.data.DataLoader(
...
file_name = file_all + 'train-cv.csv'
test_loader = torch.utils.data.DataLoader(
    mnist_load(file_name, transform=transforms.Compose([
                       ToTensor(),
                       transforms.Normalize((0.1307,), (0.3081,))
...
file_summ.index += 1
file_summ.to_csv('summision_mnist_type2_pytorch.csv',index=True,index_label=['ImageId'],header=['Label'])
print ("end")